  ...
  useEffect(() => {
    ...
    const getReviews = async () => {
      try {
        ...
        const filteredReviews = rsp.data.filter(
      ...
      {reviews.length > 0 ? (
        reviews.map((review) => (
          <ReviewCard key={review.rvNo}>
            <ReviewInfo>
              <div>
                <strong>작성일:</strong> {review.rvDate}
              </div>
              <StoreWrapper>
                <div>
                  <strong>매장명:</strong> {review.storeName}
                </div>
                <div>
                  <strong>시간:</strong> {review.rTime}:00
                </div>
              </StoreWrapper>
              <RatingWrapper>
            ...
            <ButtonContainer>
              <ReviewButton>
                <Link
                  to={`/EditReview?userId=${review.userId}&storeName=${review.storeName}&rSubmitTime=${review.rSubmitTime}&rTime=${review.rTime}&rvPrice=${review.rvPrice}&rvTaste=${review.rvTaste}&rvVibe=${review.rvVibe}&rvKind=${review.rvKind}&rvContent=${review.rvContent}`}
